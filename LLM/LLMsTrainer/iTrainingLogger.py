...
class iSummaryWriter(object):
    ...
    def create_params_dict(self, params: list) -> dict:
        ...
        Returns:
            dict: 监控变量名字典 -> {
                'loss': {'values': [0.44, 0.32, ...], 'steps': [10, 20, ...], 'index': 0},
                'reward': {'values': [10.2, 13.2, ...], 'steps': [10, 20, ...], 'index': 1},
                ...
            }
        """
        params_dict = {}
        for i, param in enumerate(params):
            params_dict[param] = {'values': [], 'steps': [], 'index': i}
    ...
    def add_scalar(self, param: str, value: float, epoch: int) -> None:
        ...
        if param not in self.params_dict:
            ...
            self.params_dict[param] = {'values': [],
                                       'steps': [], 'index': self.max_param_index}
            self.update_ax_list()

        self.params_dict[param]['values'].append(value)
        self.params_dict[param]['steps'].append(epoch)
    ...
    def record(self, dpi=200) -> None:
        ...
        for param, param_elements in self.params_dict.items():
            ...
            ax.set_xlabel('Steps')
            ax.set_ylabel(param)
            ax.plot(self.params_dict[param]['steps'],
                    self.params_dict[param]['values'],
                    color='darkorange')
