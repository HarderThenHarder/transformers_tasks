...
def evaluate_model(model, metric, data_loader, global_step):
    ...
    with torch.no_grad():
        for step, batch in enumerate(data_loader):
            ...
            metric.add_batch(pred_spans_batch=predictions, gold_spans_batch=mask_labels)
    eval_metric = metric.compute()
    model.train()
    return eval_metric['precision'], eval_metric['recall'], eval_metric['f1']
    ...
    for epoch in range(args.num_train_epochs):
        for batch in train_dataloader:
            ...
            if global_step % args.valid_steps == 0:
                ...
                precision, recall, f1 = evaluate_model(model, metric, eval_dataloader, global_step)
