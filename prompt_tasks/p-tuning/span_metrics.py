...
class SpanEvaluator(object):
    ...
    def add_batch(
        self, pred_spans_batch: List[List[str]], gold_spans_batch: List[List[str]]
    ):
        """
        ...
        """
        assert len(pred_spans_batch) == len(
            gold_spans_batch
        ...
        for pred_spans, gold_spans in zip(pred_spans_batch, gold_spans_batch):
            ...
            self.num_correct_spans += len(
                pred_set & label_set
            )  # 计算正确预测的span集合(两个集合求交集)
...
class MultiTypesSpanEvaluator(object):
    ...
    def add_batch(
        ...
        gold_spans_batch: List[List[str]],
    ):
        """
        ...
        """
        assert (
            len(types) == len(pred_spans_batch) == len(gold_spans_batch)
        ...
        for _type, pred_spans, gold_spans in zip(
            types, pred_spans_batch, gold_spans_batch
        ):
            if _type not in self.type_metric:
                self.type_metric[
                    _type
                ] = SpanEvaluator()  # 当出现一个新的type类型时，往字典中新建一个该type的span统计器
...
if __name__ == "__main__":
    ...
    metric.add_batch([["电视剧", "电影"], ["蛋糕", "面包"]], [["电视剧"], ["蛋糕", "面包", "青团", "馒头"]])
